...
==== Problem
You created a quick prototype but you found out that it needs some touch ups.
...
You can see that the attribute is gone with labels. Only facts remained. New reference was added so the reports should still be working. This might seems just like a minor thing but once you start creating more complex models with multiple stars you find this technique a necessity so why not automate it. 
...
Note: it seems it currently does not work with date references. We need to update that so I omitted it in the example so it works
