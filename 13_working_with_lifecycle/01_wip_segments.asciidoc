...
You can use life-cycle management API to create life-cycle segment, associate it with its master template project, and populate the segment with target projects that are going to receive the new versions of objects from the master template. First let's align on the terminology.
