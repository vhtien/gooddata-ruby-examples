...
Objects import happen in two stages. First you export the objects with an API call. This creates a package on our servers and provides you with a token. You can then use the token to initiate an import. In most cases you do not care about these details and 2 above methods is what you need. In some cases though you want the low level control so here is a quick example how to use those lower level methods.
